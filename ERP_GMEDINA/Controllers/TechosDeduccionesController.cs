    ...
    public class TechosDeduccionesController : Controller
    {
        ...
        public ActionResult GetData()
        {
            var otbTechosDeducciones = db.tbTechosDeducciones
                        .Select(c => new { cde_DescripcionDeduccion = c.tbCatalogoDeDeducciones.cde_DescripcionDeduccion, tede_Id = c.tede_Id, tede_RangoInicial = c.tede_RangoInicial, tede_RangoFinal = c.tede_RangoFinal, tede_Porcentaje = c.tede_Porcentaje, tede_Activo = c.tede_Activo , tede_FechaCrea = c.tede_FechaCrea }).Where(c => c.tede_Activo == true).OrderBy(c => c.tede_FechaCrea)
                        .ToList();
